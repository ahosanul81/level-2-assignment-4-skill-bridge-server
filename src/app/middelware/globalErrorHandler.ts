...
export function globalErrorHandler(
  ...
  let statusCode = 500;
  let message: string | undefined = "Somthing went wrong";
  ...
  if (err instanceof AppError) {
    ...
    const prismaClientKnownRequestError: { [key: string]: string } = {
      ...
      P2024:
        "Timed out fetching a new connection from the connection pool. (Current connection pool timeout: {timeout}, connection limit: {connection_limit})",
      P2025:
    ...
    // switch (err.code) {
    //   case "P2000":
    //     console.log(err.meta?.driverAdapterError?.cause?.originalMessage);
    //     statusCode = 400;
